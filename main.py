...
async def cmd_start(message: Message):
    ...
    if is_new:
        await message.answer(
            f"Добро пожаловать!\n\n"
            f"Вам и вашему другу начислено по {REFERRAL_BONUS} винограда!\n\n"
...
async def cmd_balance(message: Message):
    ...
    auto = "Да" if user and user['auto_collect'] else "Нет"
    double = "Да" if user and user['double_grapes'] else "Нет"
    bonus = "Да" if user and user['bonus_2h'] else "Нет"
